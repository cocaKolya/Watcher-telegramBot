...
const {
  ...
  findById,
  findRandom,
} = require('../telegramBot/botfunc');

const token = '2011450140:AAF8ryYVfPgYY_T0KOD1mzqhzgNcSpvtR0E';
...
bot.onText(/\/start/, async (msg) => {
  ...
  chatIdArr.push(chatId); //получаем идентификатор диалога, чтобы отвечать именно тому пользователю, который нам что-то прислал
...
bot.on('callback_query', async (query) => {
  ...
  if (query.data === 'randomAll') {
    // let id = Math.floor(Math.random() * (50000 - 298) + 298);
    // console.log(id);
    // let film = await findById(id);
    // console.log(film);
    // let genres = '';
